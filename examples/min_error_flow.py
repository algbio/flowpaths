...
def process_solution(graph: nx.DiGraph, model: fp.MinErrorFlow):
    if model.is_solved():
        ...
        fp.utils.draw(graph, filename="test_graph_uncorrected_graph.pdf", flow_attr="flow", draw_options={"show_edge_weights": True})
        fp.utils.draw(solution["graph"], filename="test_graph_corrected_graph.pdf", flow_attr="flow", draw_options={"show_edge_weights": True})
    else:
