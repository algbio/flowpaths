...
class kMinPathErrorCycles(walkmodel.AbstractWalkModelDiGraph):
    def __init__(
        ...
        if self.k is None:
            ...
            self.k = self.G.get_width(
                edges_to_ignore=set(self.edges_to_ignore).union(self.additional_edges),
        ...
        if trusted_edges_for_safety_percentile is not None:
            ...
            self.trusted_edges_for_safety = set(
                edge for edge in self.trusted_edges_for_safety
                if edge not in self.edges_to_ignore and edge not in self.additional_edges
            )
    ...
    def _encode_objective(self):
        ...
        if len(self.additional_edges) > 0:
            ...
            # Link integer edge_vars to used binary: if any walk uses this edge, used must be 1.
            for (u, v) in self.additional_edges:
                ub_edge = self.edge_upper_bounds[(u, v)]
                for i in range(self.k):
                    self.solver.add_constraint(
                        self.edge_vars[(u, v, i)] <= ub_edge * self.additional_edges_used_vars[(u, v)],
                        name=f"edge_used_link_u={u}_v={v}_i={i}",
                    )

        self.solver.set_objective(
            self.solver.quicksum(self.path_slacks_vars[(i)] for i in range(self.k))
            + self.additional_edges_lambda
            * self.solver.quicksum(
                self.additional_edges_used_vars[(u, v)] for (u, v) in self.additional_edges
            ),
            sense="minimize"
        )
    ...
    def is_valid_solution(self, tolerance=0.001):
        ...
        obj_tolerance = tolerance * self.k
    ...
    def get_objective_value(self):
        ...
        # sum of slacks
        objective_value = sum(self._solution["slacks"])

        if self.additional_edges_lambda > 0 and len(self.additional_edges) > 0:
            additional_edges_used_sol = self.solver.get_values(self.additional_edges_used_vars)
            objective_value += self.additional_edges_lambda * sum(
